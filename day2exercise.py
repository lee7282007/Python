...
for f in files:
    if f.endswith('jpg') or f.endswith('png') or f.endswith('gif'):
        ...
        shutil.move(os.path.join(path, f), path + '/image')
    if f.endswith('doc') or f.endswith('docx') or f.endswith('md') or f.endswith('ppt'):

        shutil.move(os.path.join(path, f), path + '/document')
